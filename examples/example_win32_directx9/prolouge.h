...
#define IM_VEC2_CLASS_EXTRA operator float2() const {return {x, y};} \
ImVec2(float2 v) {x = v.x; y = v.y; }
...
bool init() {
    ...
    ::RegisterClassEx(&wc);
    hwnd = ::CreateWindow(wc.lpszClassName, _T("Evolution Simulator"), WS_OVERLAPPEDWINDOW, 100, 100, 1280, 800, NULL, NULL, wc.hInstance, NULL);
...
void fullscreen_dockspace() {
    ...
    ImGui::Begin("Evolution Simulator", NULL, window_flags);
    ImGui::PopStyleVar();
    ImGui::PopStyleVar(2);

    ImGuiID dockspace_id = ImGui::GetID("fullscreen dockspace");
